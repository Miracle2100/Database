...
SELECT code, budget FROM departments ORDER BY budget DESC LIMIT 1 OFFSET 1;
...
UNION SELECT name, lastname FROM customers WHERE city = 'Almaty';
...
UPDATE employees
SET department = (SELECT code FROM departments WHERE departments.name = 'IT')

WHERE department = (SELECT code FROM departments WHERE departments.name = 'Research');

DELETE FROM employees WHERE department = (SELECT code FROM departments WHERE departments.name = 'IT');
